...
interface IYandexMoney {
     ...
     * @abstract
     * @param $clientId string идентификатор приложения в системе Яндекс.Деньги
     * @param $clientSecret string OAuth2 идентификатор приложения в системе Яндекс.Деньги
     ...
     */
    public function __construct($clientId, $certificateChain, $clientSecret = '');
...
class YandexMoney implements IYandexMoney {
    ...
    public function receiveOAuthToken($code, $redirectUri) {
		...
		if ($this->clientSecret)
			$paramArray['client_secret'] = $this->clientSecret;
